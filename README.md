...
| 3 | 价格弹性分三个层级，以补贴盈亏临界线 β=−1 为参照 | 电视 **−2.65** 显著超过（p=0.036）/ 手机 −1.18 紧贴临界线（CI 跨过 −1）/ 其余合并 −0.24 明显低于（t=5.31） |
| 4 | 最优分配由电视主导，手机只是电视封顶后的预算溢出 | 优化 **+1.51%** GMV；电视 ROI **1.44**，手机仅 **0.17**，整体 **0.30** |
...
电视吃到 10% 的补贴率上限（ROI 1.44），剩下的预算才流向手机（ROI 0.17），其余七个品类为零。**手机拿到补贴不是因为模型判定它划算，而是电视封顶后预算花不完的溢出**——这也是建议里把补贴规模压到 GMV 0.5% 左右的原因。该结论对「其他品类」那个不显著的弹性估计不敏感——β 取 −0.24 / 0 / −0.52（CI 下界）三种设定，最优解完全相同。
...
**检验的是 β 与 −1 的距离，而不是 β 与 0。** 补贴能否拉高 GMV 取决于 |β| > 1，而非「弹性显著不为零」。合并后的「其他品类」估计为 −0.24，p=0.093，不显著异于 0——但它显著**弱于** −1（t=5.31，p<0.0001），而后者才是分配决策需要的方向。因此本项目不主张品类间弹性的倍数关系（该比值的分母不显著），只主张各品类相对临界线的位置。
...
│   ├── day2_robustness.py        显著性检验与最优解稳健性检查
...
python3 scripts/day2_robustness.py
...
价格弹性基于观察性数据，商品固定效应无法排除随时间变化的混淆因素，结论定位为方向性参考，落地需 A/B 实验验证；其余七个品类合并后的弹性估计不显著（−0.24，p=0.093），故不报告品类间弹性的倍数关系，只报告各品类相对 β=−1 临界线的位置（已验证最优解对该估计不敏感）；优化模型假设品类间需求独立，忽略替代效应，GMV 增量为上界；恒弹性假设仅在数据覆盖的价格波动范围内成立；毛利率为外生假设，已做四档敏感性分析；仅一个月数据，无法识别季节性。详见 [PROJECT_SUMMARY.md](PROJECT_SUMMARY.md)。
